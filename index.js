...
bot.on("ready", () => {
    ...
    bot.user.setActivity('Тестовые команды,возможны баги', { type: 'PLAYING' });
			...
			if((msg.id == game.msg0.id || msg.id == game.msg1.id) && game.stage < stages.length) {
				...
				if(game.guesses.indexOf(letter) == -1) {
					...
					if(game.phrase.indexOf(letter) == -1) {
						...
						if(sik) {
                            game.msg0.edit(stages[game.stage].replace("o", "o ~ Верно!Игра окончена"));
                        }
